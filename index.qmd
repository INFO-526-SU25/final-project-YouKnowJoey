...
longbeach_clean |>
  ...
  ggplot(aes(x = n, y = reorder(animal_type, n), fill = species_group)) +
    ...
    theme(
      ...
      plot.caption = element_text(margin = margin(t = 9), size = 18),
      legend.position = "inside",
      legend.position.inside = c(0.9, 0.5),
      legend.background = element_rect(fill = "white", color = "grey75",size   = 0.25)
    ) + 
    theme(
      plot.title = element_text(size = 30, face = "bold", hjust = 0.1),
      plot.subtitle = element_text(size = 20, hjust = 0.055),
