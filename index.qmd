...
adopt_df_full <- longbeach_clean |>
  ...
  mutate(intake_year = factor(intake_year, levels = 2017:2024))

# 95th percentile to make the grap display better
...
ggplot() +
  ...
  geom_col(
    data = count_df,
    aes(x = intake_year, y = n_adopt * scaleFact),
    fill  = "grey80",
    width = 0.6
  ) +
  # violins
  geom_violin(
    data   = adopt_df_full,
    aes(x = intake_year, y = days_to_adoption),
    trim   = FALSE, 
    fill   = "skyblue", 
    color  = NA, 
    alpha  = 0.7
  ...
  scale_y_continuous(
    ...
    labels = comma_format(),
    expand = c(0, 0),                    # <-- remove bottom padding
    sec.axis = sec_axis(
      ~ . / scaleFact,
  ...
  labs(
    ...
    subtitle = "Violins = days to adoption; Bars = count of adoptions (2017–2024)",
    x        = "Intake Year"
  ) +
  theme_minimal() +
  theme(
